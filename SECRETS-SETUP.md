...
Hostinger hPanel, File Manager. Open `domains/condolencecardmessages.com/`. You will see `public_html` there. Create a new folder NEXT TO it (not inside it) called:

```
condolencecardmessages_private
```

The result should look like:

```
domains/condolencecardmessages.com/
    public_html/            <- the website (git deploys here)
    condolencecardmessages_private/  <- your secrets and the leads database
```

## Step 2: create secrets.php inside that folder

Inside `condolencecardmessages_private`, create a file called `secrets.php`. Copy the contents of `SECRETS-TEMPLATE.php` from the repo into it, then fill in the three values:
...
- `NOTIFY_EMAIL`: `info@condolencecardmessages.com` (create the mailbox in hPanel, Emails; forward it to Gmail if you like).

Save.

## Step 3: check it worked

Visit `https://condolencecardmessages.com/leads.php`. You should see a normal password prompt. If you instead see the message "Admin is locked because the secrets file is missing", the file is in the wrong place or has a typo: recheck the folder name and that the file returns an array.
