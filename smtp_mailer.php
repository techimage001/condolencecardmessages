...
function smtp_send($cfg, $to, $subject, $body, &$error = null) {
    ...
    $from = $cfg['from']; $fromName = $cfg['from_name'] ?? 'Condolence Card Messages';
    $timeout = 20;
    ...
    $cmd("EHLO condolencecardmessages.com"); $r = $read();
...
function send_mail_smart($secrets, $to, $subject, $body) {
    $from = $secrets['from_email'] ?? 'info@condolencecardmessages.com';
    if (!empty($secrets['smtp_pass'])) {
        $cfg = [
            ...
            'from' => $from,
            'from_name' => $secrets['from_name'] ?? 'Condolence Card Messages',
        ];
        $err = null;
