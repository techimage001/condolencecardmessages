# Condolence Card Messages (condolencecardmessages.com)
...
- `contact.html` (info@condolencecardmessages.com), `privacy.html`, `terms.html`.
...
### 2. The mailbox
Create `info@condolencecardmessages.com` in hPanel (Emails), forward it to Gmail if you like. It receives the contact form address, privacy requests and a notification for every VERIFIED signup, which doubles as an offsite backup of the list.
...
7. Search Console: add the property, submit `https://condolencecardmessages.com/sitemap.xml`.
...
- Quotes are original to Condolence Card Messages, or verified public domain with the real source named. Never add lines attributed to modern authors or celebrities, and never copy collections from other sites or apps.
